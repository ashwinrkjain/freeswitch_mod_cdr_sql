/*
 * Contributor(s):
 * Ashwin Jain <ashwinrkjain@gmail.com>
 */
...
SWITCH_MODULE_LOAD_FUNCTION(mod_cdr_sql_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_cdr_sql_shutdown);
SWITCH_MODULE_DEFINITION(mod_cdr_sql, mod_cdr_sql_load, mod_cdr_sql_shutdown, NULL);
...
static switch_status_t config_validate_spool_dir(switch_xml_config_item_t *item, const char *newvalue, switch_config_callback_type_t callback_type, switch_bool_t changed)
{
	if ((callback_type == CONFIG_LOAD || callback_type == CONFIG_RELOAD)) {
		if (zstr(newvalue)) {
			globals.spool_dir = switch_core_sprintf(globals.pool, "%s%scdr-sql", SWITCH_GLOBAL_dirs.log_dir, SWITCH_PATH_SEPARATOR);
		}
	}
...
static switch_xml_config_item_t config_settings[] = {
	...
	SWITCH_CONFIG_ITEM_STRING_STRDUP("db-username", CONFIG_RELOADABLE, &globals.db_username, "root", NULL, NULL),
	SWITCH_CONFIG_ITEM_STRING_STRDUP("db-password", CONFIG_RELOADABLE, &globals.db_password, "password", NULL, NULL),
	SWITCH_CONFIG_ITEM_STRING_STRDUP("db-database", CONFIG_RELOADABLE, &globals.db_database, "cdrdb", NULL, NULL),
	SWITCH_CONFIG_ITEM("db-port", SWITCH_CONFIG_INT, CONFIG_RELOADABLE, &globals.db_port, 3306, NULL, NULL, NULL),
...
static switch_status_t insert_cdr(const char *values)
{
	...
	res = mysql_real_query(globals.db_connection, sql, strlen(sql));
	if (res) {
